	...
	if (isForced) then
		if (player:IsAdmin()) then
			...
			if (istable(rankTable) and targetRankTable.position and targetRankTable.position != rankTable.position) then
				...
				Clockwork.player:NotifyAdmins("o", {"PlayerForceDemoted", player:Name(), target:Name(), (target:GetFactionRank())});
			else
		...
		if (player:GetFaction() == target:GetFaction()) then
			if (Clockwork.player:CanDemote(player, target)) then
				...
				Clockwork.player:NotifyAdmins("o", {"PlayerDemotedPlayer", player:Name(), target:Name(), (target:GetFactionRank())});
			else
