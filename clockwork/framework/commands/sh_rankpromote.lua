	...
	if (isForced) then
		if (player:IsAdmin()) then
			...
			if (istable(rankTable) and targetRankTable.position and targetRankTable.position != rankTable.position) then
				...
				Clockwork.player:NotifyAdmins("o", {"PlayerForcePromoted", player:Name(), target:Name(), (target:GetFactionRank())});
			else
		...
		if (player:GetFaction() == target:GetFaction()) then
			if (Clockwork.player:CanPromote(player, target)) then
				...
				Clockwork.player:NotifyAdmins("o", {"PlayerPromotedPlayer", player:Name(), target:Name(), (target:GetFactionRank())});
			else
