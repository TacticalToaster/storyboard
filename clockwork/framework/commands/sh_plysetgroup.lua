...
function COMMAND:OnConsoleRun(arguments)
	...
	if (target) then
		if (!Clockwork.player:IsProtected(target)) then
			Clockwork.player:NotifyAdmins("o", {"PlayerSetPlayerGroupTo", L("ConsoleUser"), target:Name(), userGroup});
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		if (!Clockwork.player:IsProtected(target)) then
			Clockwork.player:NotifyAdmins("o", {"PlayerSetPlayerGroupTo", player:Name(), target:Name(), userGroup});
