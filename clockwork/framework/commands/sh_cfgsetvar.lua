			...
			if (value != nil) then
				...
				if (configObject("isPrivate")) then
					if (configObject("needsRestart")) then
						Clockwork.player:NotifyAdmins("o", {"PlayerConfigSetNextRestart", player:Name(), keyPrefix, key, string.rep("*", string.utf8len(printValue))});
					else
						Clockwork.player:NotifyAdmins("o", {"PlayerConfigSet", player:Name(), keyPrefix, key, string.rep("*", string.utf8len(printValue))});
					end;
				elseif (configObject("needsRestart")) then
					Clockwork.player:NotifyAdmins("o", {"PlayerConfigSetNextRestart", player:Name(), keyPrefix, key, printValue});
				else
					Clockwork.player:NotifyAdmins("o", {"PlayerConfigSet", player:Name(), keyPrefix, key, printValue});
				end;
			else
