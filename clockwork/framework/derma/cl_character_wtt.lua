...
function PANEL:Init()
	...
	self.pointsUsed:SetMouseInputEnabled(true);
...
function PANEL:Think()
	...
	if (input.IsMouseDown(MOUSE_FIRST)) then
		if (self.pointsUsed:IsHovered()) then
			local mx = gui.MouseX();
			local my = gui.MouseY();

			local px, py = self.pointsUsed:LocalToScreen(0, 0); // FIGURE THIS OUT

			local mousePercent = (mx - px) / self.pointsUsed:GetWide();

			local setPoints = math.Clamp(math.Round(mousePercent * self.attributeTable.maximum), 0, self.attributeTable.maximum);

			local pointsUsed = self:GetPointsUsed();
			local points = math.max(self.maximumPoints - pointsUsed, 0);

			print(setPoints, mousePercent, mx, px, self.pointsUsed:GetWide());

			self.totalPoints = math.Clamp(math.Approach(self.totalPoints, setPoints, math.max(self.totalPoints - setPoints, 0) + points), 0, self.attributeTable.maximum);
		end;
	end;
	...
	markupObject:Title("Add Points");
	markupObject:Add("Shift+Ctrl 100");
	markupObject:Add("Shift 10");
	markupObject:Add("Ctrl 5");

	self.addButton:SetMarkupToolTip(markupObject:GetText());

	local markupObject = Clockwork.theme:GetMarkupObject();

	markupObject:Title("Remove Points");
	markupObject:Add("Shift+Ctrl 100");
	markupObject:Add("Shift 10");
	markupObject:Add("Ctrl 5");

	self.removeButton:SetMarkupToolTip(markupObject:GetText());
end;

-- A function to add a point.
function PANEL:AddPoint(points)
	local pointsUsed = self:GetPointsUsed();
	local points = math.Clamp(self.maximumPoints - pointsUsed, 0, points);
	
	//if (pointsUsed + points <= self.maximumPoints) then
		self.totalPoints = math.Approach(self.totalPoints, self.attributeTable.maximum, self.totalPoints + points);//math.Approach(self.totalPoints, self.maximumPoints, self.totalPoints + points);
	//end;
end;

-- A function to remove a point.
function PANEL:RemovePoint(points)
	self.totalPoints = math.max(self.totalPoints - points, 0);
end;
